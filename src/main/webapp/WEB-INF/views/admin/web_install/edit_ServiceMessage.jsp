		...
		<div class="listbox">
			<div class="pagetitle">客服消息编辑
				...
				<table class="table table-bordered table-condensed">
					<tr>
						<td colspan="2">客服消息编辑</td>
					</tr>
					<tr>
						<td class="item_title">发送对象</td>
						<td class="item_input">
							<input type="text" name="openid" id="openid" value="${openid}" class="form-control input-sm">
							<%-- <c:if test="${empty telephone}">
								<input type="text" name="mobile" id="mobile" placeholder="请输入手机号码" class="form-control input-sm">
							</c:if>
							<c:if test="${!empty telephone}">
								<input type="text" name="mobile" id="mobile" value="${telephone}" class="form-control input-sm">
							</c:if> --%>
						</td>
					</tr>
					<tr>
...
<script type="text/javascript"> 
	...
	function editUp(){ 
		...
		var param = {"openid":openid,"content":content,"msgtype":msgtype,"serviceId":1}
		//alert(JSON.stringify(param))
		$.ajax({
				type:"post",
				url:"admin/send_oneserviceMessage",
				data:param,
